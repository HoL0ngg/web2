  ...
  fetch("handles/getSession.php")
    ...
    .then((data) => {
      if (data.success) {
        ...
        document
          .getElementById("taikhoan-container")
    ...
    .then((data) => {
      if (data.success) {
        document.getElementById(
          "taikhoan-container"
        ).innerHTML = `Xin chào, ${data.username}`;
      } else {
        document.getElementById(
          "taikhoan-container"
        ).innerHTML = `<i class="fa-solid fa-user" style="color: #6794c1;"></i>
      ...
      fetch("handles/RegisterController.php", {
        ...
        .then((data) => {
          ...
          if (data.success) {
            // console.log(data);
            setTimeout(() => {
              document.getElementById("register-wrapper").style.display =
                "none";
      ...
      fetch("handles/handleChangePassword.php", {
        ...
        .then((data) => {
          ...
          if (data.success) {
            ...
            setTimeout(() => {
              let taikhoanContainer =
                document.getElementById("taikhoan-container");
              ...
              document.getElementById("changepassword-wrapper").style.display =
                "none";
              document.getElementById("login-wrapper").style.display = "block";
              document.getElementById("login-wrapper").style.backdropFilter =
                "brightness(0.8)";
            }, 2000);
          }
        })
  ...
  fetch("XuLyYeuThich.php", {
    ...
    .then((data) => {
      ...
      data.data.forEach((productId) => {
        const heartIcon = [...heartIcons].find(
          (icon) => icon.closest(".product").dataset.id == productId.product_id
        );
...
async function loadProducts(pagenum = 1, isLove = false) {
  ...
  for (let i = 0; i < checkboxes_loaisanpham.length; i++) {
    if (checkboxes_loaisanpham[i].checked) {
      selected_checkboxes_loaisanpham.push(
        parseInt(checkboxes_loaisanpham[i].value)
      );
    }
  }

  const response = await fetch(
    `api/pagination_api.php?pagenum=${pagenum}&keyword=${encodeURIComponent(
      keyword
    )}&selected_checkboxes_brand=${encodeURIComponent(
      JSON.stringify(selected_checkboxes_brand)
    )}&selected_checkboxes_loaisanpham=${encodeURIComponent(
  ...
  data.products.forEach((product) => {
    s += `<div class="product" data-id="${product.product_id} ">
        ...
        <div class="product-img">
            <img src="${
              product.image_url
            }" alt="img1" onclick="getInfoProduct(${product.product_id})">
        </div>
        ...
        <div style="margin: 7px;">
                <button class="product-detail-button" onClick='getInfoProduct(${
                  product.product_id
                })'>Chi tiết</button>
        </div>
    </div>`;
...
function showToastForCart(message, isSuccess) {
  Swal.fire({
    icon: isSuccess ? "success" : "error",
    title: isSuccess ? "Thành công!" : "Lỗi!",
    text: message,
...
addToCart = (id, quantity) => {
  fetch("cart.php", {
    ...
    .then((res) => res.json())
    .then((data) => {
      showToastForCart("Thêm vào giỏ hàng thành công", true);
      updateCartCount();
    })
    .catch((err) => {
